...
import { installToolBrew, checkBrewAvailable, type BrewInstallEvent } from "../api/installation-commands";

export interface BrewInstallState {
...
export function useBrewInstall(options?: {
  onSuccess?: () => void;
  onError?: (error: string) => void;
}) {
  const [state, setState] = useState<BrewInstallState>({
    isInstalling: false,
    output: [],
    error: null,
    success: false,
  ...
  const install = useCallback(async (caskName: string) => {
    setState({
      isInstalling: true,
      output: [],
      error: null,
    ...
    try {
      ...
      await checkBrewAvailable();

      await installToolBrew(caskName, (event: BrewInstallEvent) => {
        setState((prev) => {
          switch (event.type) {
            case "status":
              return {
