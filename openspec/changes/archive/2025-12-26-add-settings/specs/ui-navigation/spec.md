...
- **AND** shows a tab/button for each available feature (CLI Tools, Software, Settings, and future features)
- **AND** each tab displays an icon and label
- **AND** the currently active tab is visually highlighted
- **AND** the "Environment" tab is no longer displayed (functionality moved to Settings)
